...
void _li_init(){
	...
	G_bus.main = NULL;
	G_bus.get8 = &_LIMPBus_g_get8;
	G_bus.get16 = &_LIMPBus_g_get16;
	G_bus.get32 = &_LIMPBus_g_get32;
	G_bus.set8 = &_LIMPBus_g_set8;
	G_bus.set16 = &_LIMPBus_g_set16;
	G_bus.set32 = &_LIMPBus_g_set32;
	
	LIMP_CPU_init(Gm_cpu);
	G_cpu.pins.device_bus = &G_bus;
	G_cpu.pins.devide_pci = &G_pci;
...
Luint32 _li_execute(char* *cmd){
	...
	else if(_li_se(buffer, "step")){
		LIMP_CPU_step(Gm_cpu);
		if(_li_feedback){
			LIMP_CPU_log(Gm_cpu);
		}
		return 0;
